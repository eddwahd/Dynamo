function [dynlist,errorlist,T_list] = demo_resonance_ehc_v5(detuning)
...
% V5 -- now writing wrapper function for finding smallest amount of Time
% (T) such that fidelity is still reasonable.
...
rabifreq = 2*pi*0.002; % in GHz (= 10 MHz)
T = 1/rabifreq*2*pi; % total time 
T_list = linspace(T,10*T,T_listsize);

pd = makedist('Normal');
x = linspace(-1,1,ensemblesize);
weights = we_noise*pdf(pd,x);
...
dynlist = cell(1,length(T_list));
errorlist = zeros(1,length(T_list));
for tt = 1:length(T_list)
    ...
    if 1
        ...
        dyn.system.weight = weights;
        % end of code added by ehchen, 2014-11-05
    end
    
    TimeBins = 21; % should be an odd number.. usually
    ...
    dyn.analyze();
    dynlist{tt} = dyn;
    errorlist(tt) = dyn.stats.error(end);
end
figure(333);
semilogy(T_list,1-errorlist);
xlabel('time');
ylabel('fidelity');
end
